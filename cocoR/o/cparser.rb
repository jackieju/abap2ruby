class CParser < CRRParser
   ...
   def Name()
      ...
      if @sym==C_identifierSym
         ...
         if @sym==C_DATASym||@sym==C_MESSAGESym
            AllowedKeywords()
         else
            GenError(708)
         end
   ...
   def VariableDefineStatement()
      ...
      if @sym>=C_identifierSym&&@sym<=C_DATASym||@sym==C_MESSAGESym||@sym==C_BEGINSym||@sym==C_ENDSym
         if @sym==C_BEGINSym
            Get()
            Expect(C_OFSym)
            if @sym==C_COMMONSym
            ...
            if @sym==C_ENDSym
               ...
               if @sym>=C_identifierSym&&@sym<=C_DATASym||@sym==C_MESSAGESym
                  Name()
   ...
   def stTRY()
      ...
      while (@sym==C_CATCHSym)
         ...
         while (@sym>=C_identifierSym&&@sym<=C_DATASym||@sym==C_MESSAGESym)
            Name()
               ...
               if @sym==C_LparenSym||@sym==C_PlusSym||@sym>=C_MinusGreaterSym&&@sym<=C_INCREMENTSym
                  if @sym==C_LparenSym||@sym==C_TOSym||@sym==C_PlusSym
                     ...
                     if @sym>=C_MinusGreaterSym&&@sym<=C_EqualGreaterSym
                        ...
                        if @sym>=C_identifierSym&&@sym<=C_DATASym||@sym==C_StarSym||@sym==C_MESSAGESym
                           if @sym==C_StarSym
                              Get()
                           else
                              if @sym>=C_identifierSym&&@sym<=C_DATASym||@sym==C_MESSAGESym
