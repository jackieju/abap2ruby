class CParser < CRRParser
   ...
   def TypeDes()
      ...
      when C_numberSym,
         ...
         if @sym==C_PointSym||@sym>=C_LENGTHSym&&@sym<=C_DECIMALSSym||@sym>=C_READMinusONLYSym&&@sym<=C_OCCURSSym||@sym==C_VALUESym
            if @sym==C_LENGTHSym
               Get()
               Expression()
            end
